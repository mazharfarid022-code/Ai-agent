...
import './App.css';
import { generateRealLeads } from "./services/apollo";
...
function App() {
  ...
  const handleGenerateLeads = useCallback(async(count: number, source?: string) => {
    setIsGenerating(true);
  try {
  const newLeads = await generateRealLeads({
    count,
    source,
  });
  ...
  toast.success(`${newLeads.length} leads generated successfully!`);
